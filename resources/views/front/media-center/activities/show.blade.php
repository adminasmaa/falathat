...
@section('content')
    <!--Page Header-->
    @include('front.includes.page-header', ['currentPage' => $settings['Translations']['media_center_activity_details']])
            ...
            <div class="row">
                <div class="col-lg-8 col-md-7">
                    <div class="news_item shadow-equal">
                        <div class="image">
                            <img src="{{ Storage::url($activity->image) }}" alt="Latest News" class="img-responsive">
                        </div>
                        <div class="news_desc text-center text-md-right">
                            <h3 class="text-capitalize font-normal darkcolor">
                                <a href="#!"> {{ $activity->title('ar') }} </a></h3>
                            <ul class="meta-tags top20 bottom20">
                                <li><a href="#."><i
                                            class="fas fa-calendar-alt"></i>{{ \App\Models\Activity::DataFormat($activity->created_at) }}
                                    </a></li>
                                <li><a href="#."> <i class="far fa-user"></i> {{ $activity->author('ar') }} </a></li>
                                <li><a href="#."><i
                ...
                <div class="col-lg-4 col-md-5">
                    <aside class="sidebar whitebox mt-5 mt-md-0">
                        ...
                        <div class="widget heading_space_half shadow-equal wow fadeIn" data-wow-delay="350ms">
                            <h4 class="text-capitalize darkcolor bottom20 text-center text-md-right">{{ $settings['Translations']['latest_activities'] }}</h4>
                            @foreach($latest_activities as $single_activity)
                                <div class="single_post d-table bottom15">
                                    <a href="{{ route('front.news.show', $single_activity) }}" class="post"><img
                                            style="width: 50px;height: 54px;"
                                            src="{{ Storage::url($single_activity->thumbnail) }}"
                                            alt="post image"></a>
                                    <div class="text text-right px-2">
                                        <a href="{{ route('front.news.show', $single_activity) }}">{{ $single_activity->title('ar') }}</a>
                                        <span>{{ \App\Models\Activity::DataFormat($single_activity->created_at, true) }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
